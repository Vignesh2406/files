...
public class PlaintextScanner {
	...
	/**
	 * This method returns an ArrayList<String> containing the .txt that is to
	 * be read.
	 * 
	 * @return ArrayList<String> plainTextMessage - The entire text file, saved
	...
	public ArrayList<String> getPlainText() throws IOException {
		...
		inFile.close();
		// System.out.println("bbb "+plainTextMessage);
		...
		for (String s : plaintext) {
			for (int i = 0; i < s.length(); i++) {
				...
				if (letter >= 'a' && letter <= 'z') {
					...
					// System.out.println("letter - 'a' "+(letter - 'a'));
