  ...
  implements Comparable<Picture>
{
  ...
  /**
   * Method to compare a picture to another picture
   * @param p the picture to compare to
   * @return a negative number if this picture is less than
   * the passed picture, 0 if they are equal, and a positive
   * number if this picture is greater than the passed picture
   * This method compares widths, then heights, then fileNames
   */
  public int compareTo(Picture p)
  {
  
    // first compare the widths
    int diffWidth = this.getWidth() - p.getWidth();
    
    // if the difference in widths isn't zero return it
    if (diffWidth != 0)
      return diffWidth;
    
    // compare the heights
    int diffHeight = this.getHeight() - p.getHeight();
   
    // if the difference in heights isn't zero return it
    if (diffHeight != 0)
      return diffHeight;
    
    // else compare the fileNames
    return this.getFileName().compareTo(p.getFileName());
  }
  
  /**
   * Method to check if two pictures are equal
   * @param p the picture to compare to
   * @return true if the same size and fileName else false
   */
...
public void copyFlowersBetter()
{
 ...
 /**
   ...
   * rectangles on the current picture
   */
  public void fillRectangle(int x, int y, 
                            int width, 
                            int height, 
                            Color rectColor)
  {
  ...
  public static Picture drawSlantedSquares(int smallestLength)
  {
    Picture pict = new Picture (600, 600);
    Point p1 = new Point (0,0);
    Point p2 = new Point (0, 600);
    Point p3 = new Point (600, 600);
    Point p4 = new Point (600, 0);
    SlantedSquare s1 = new SlantedSquare (p1, p2, p3, p4);
    ...
    s1.subdivideSquare(g, smallestLength);
    return pict;
  }
  ...
  /**
   * Method to draw a grid of 3 lines on the
   * picture using a Turtle object
   */
  public void drawGrid2()
  {
    ...
    turtle1.penUp();
    turtle1.turn(180);
    int xDist = this.getWidth() / 4;
    int yDist = this.getHeight() / 4;
   
    // draw vertical lines
    turtle1.moveTo(xDist,0);
    turtle1.penDown();
    turtle1.forward(this.getHeight());
    turtle1.penUp();
    turtle1.moveTo(xDist * 2, 0);
    turtle1.penDown();
    turtle1.forward(this.getHeight());
    turtle1.penUp();
    turtle1.moveTo(xDist * 3, 0);
    turtle1.penDown();
    turtle1.forward(this.getHeight());
    
    // draw horizontal lines
    turtle1.penUp();
    turtle1.moveTo(0,yDist);
    turtle1.penDown();
    turtle1.turnLeft();
    turtle1.forward(this.getWidth());
    turtle1.penUp();
    turtle1.moveTo(0,yDist * 2);
    turtle1.penDown();
    turtle1.forward(this.getWidth());
    turtle1.penUp();
    turtle1.moveTo(0,yDist * 3);
    turtle1.penDown();
    turtle1.forward(this.getWidth());
  }
  ...
  public void chromakeyBlack()
  {
    ...
    for (int x=0; x<getWidth(); x++) 
    {
      ...
      for (int y=0; y<getHeight(); y++) 
      {
        ...
        /* if the color at the current pixel is white, then make it blackr
         */

        if (currPixel.getRed() == 255 && currPixel.getGreen() == 255 && currPixel.getBlue ()== 255)
        {
  ...
  /**
   * Method to skid a picture (copy from one x to the rest
   * of the x values) horizontally starting
   * at the specified percentage
   * @param percentage the percentage of the picture to copy
   * before the skid
   */
  public void horizontalSkid(double percentage)
  {
    int stopPoint = (int) (this.getWidth() * percentage);
    Pixel orig = null;
    Pixel copy = null;
    
    // start at the stop point
    for (int x = stopPoint; x < this.getWidth(); x++)
    {
      for (int y = 0; y < this.getHeight(); y++) 
      {
        orig = this.getPixel(stopPoint,y);
        copy = this.getPixel(x,y);
   ...
   * before the skid
   */
  public void verticalSkid(double percentage)
  {
    int stopPoint = (int) (this.getHeight() * percentage);
    Pixel orig = null;
    Pixel copy = null;
    
    // start at the stop point
    for (int y = stopPoint; y < this.getHeight(); y++)
    {
      for (int x = 0; x < this.getWidth(); x++) 
      {
        orig = this.getPixel(x,stopPoint);
        copy = this.getPixel(x,y);
        copy.setColor(orig.getColor());
      }
    }
  ...
  /**
   * Method to calculate the new red value using 
   * the relief algorithm
   * @param currPixel the current pixel
   * @param diagPixel the diagonal pixel
   */
  private int getRedReliefValue(Pixel currPixel, 
                                Pixel diagPixel)
  {
    int oldRed = currPixel.getRed();
    int newValue = oldRed + (255/2 - diagPixel.getRed());
    if (newValue > 255)
       newValue = 255;
    if (newValue < 0)
       newValue = 0;
  ...
  /**
   * Method to make a relief looking picture 
   * in greyscale 
   * based on an algorithm in Beyond Photography
   * by Gerald Holzmann - ISBN 0-13-074410-7 
   */
  public void relief() 
  {
    // first make the picture gray
    grayscale();
    
    // calculate the max x and y
    int maxX = this.getWidth() - 2;
    int maxY = this.getHeight() - 2;
    
    // declare the pixels
    Pixel currPixel, diagPixel = null;
    ...
    for (int x = 0; x < maxX - 2; x++) 
    {
      ...
      {
        currPixel = this.getPixel(x,y);
        diagPixel = this.getPixel(x+2,y+2);
        newValue = getRedReliefValue(currPixel,diagPixel);
        currPixel.setColor(new Color(newValue,newValue,newValue));
      }
    }
  }
  
  public Picture brightnessShift()
  {
    int maxShift = 255/4;
    
    // create a new picture bigger in y by the max shift amount
    Picture newPict = new Picture(this.getWidth(),
                                  this.getHeight() + maxShift);
    
    // declare the pixels
    Pixel currPixel, newPixel = null;
    
    // change to grayscale
    grayscale();
    
    // declare the new y index
    int newY = 0;
    ...
    for (int x = 0; x < this.getWidth(); x++)
    {
      ...
      {
        currPixel = this.getPixel(x,y);
        newY = y + (currPixel.getRed()/4);
        if (newY >= 0 && newY < newPict.getHeight())
        {
          newPixel = newPict.getPixel(x,newY);
          newPixel.setColor(currPixel.getColor());
        }
      }
    }
  ...
  /**
   * Method to project the picture onto a cone
   * in the center of the picture
   * @return the new projected picture
   */
  public Picture coneProjection()
  {
    int width = this.getWidth();
    int height = this.getHeight();
    Picture newPict = new Picture(this.getWidth(),
                                  this.getHeight());
    double radius, angleInRadians = 0;
    int centerX = width / 2;
    int centerY = height / 2;
    Pixel currPixel, newPixel = null;
    int xDiff,yDiff = 0;
    int maxR = Math.max(centerX,centerY);
    int newX, newY;
    double newRadius;
    int xCart,yCart = 0;
    int newXCart,newYCart = 0;
    ...
    for (int x = 0; x < width; x++) {
      for (int y = 0; y < height; y++) {
        currPixel = this.getPixel(x,y);
        xCart = x - centerX;
        yCart = height - y - centerY;
        radius = Math.sqrt((xCart*xCart) + (yCart*yCart));
        angleInRadians = Math.acos(xCart/radius);
        newRadius = (Math.sqrt(radius * maxR));
        newXCart = (int) (Math.cos(angleInRadians) * newRadius);
        newYCart = (int) (Math.sin(angleInRadians) * newRadius);
        
        if (y > centerY)
          newYCart = newYCart * -1;
        newX = newXCart + centerX;
        newY = height - newYCart - centerY;
                       
        if (newY >= 0 && newY < height &&
            newX >= 0 && newX < width) 
        {
           newPixel = newPict.getPixel(newX,newY);
           newPixel.setColor(currPixel.getColor());
        }
  ...
  public Picture oilPaint(int dist) {
    ...
    // loop through the pixels
    for (int x = 0; x < copyPict.getWidth(); x++) {
      for (int y = 0; y < copyPict.getHeight(); y++) {
        copyPixel = copyPict.getPixel(x,y);
        retPixel = retPict.getPixel(x,y);
        retPixel.setColor(copyPixel.getMostCommonColorInRange(dist));
      }
    }
    return retPict;
  }
